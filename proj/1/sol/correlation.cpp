 ...
 *                      a special case is selection is a NULL pointer, which 
 *                      means all the pixels should be filtered.
 *
 *  OUTPUT:
...
void image_filter(double* rsltImg, const unsigned char* origImg, 
                  const unsigned char* selection, int imgWidth, int imgHeight,
                  ...
                  double scale, double offset)
{
  ...
  for (int col = 0; col < imgWidth; col++) {
    for (int row = 0; row < imgHeight; row++) {
      idx = 3*(row*imgWidth + col);
      if (selection == NULL || selection[idx] == 1) {
        pixel_filter(rsltImg + idx, col - (knlWidth/2), row - (knlHeight/2),
 ...
 *    knlHeight:    the height of the kernel
 *
 *    scale, offset:  after correlating the kernel with the origImg, the result 
 *            pixel should be divided by scale and then added by offset
 *
 *  OUTPUT:
...
void pixel_filter(double rsltPixel[3], int x, int y, 
                  const unsigned char* origImg, int imgWidth, int imgHeight,
                  const double* kernel, int knlWidth, int knlHeight, 
                  double scale, double offset)
{
