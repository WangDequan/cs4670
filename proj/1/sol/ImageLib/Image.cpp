...
void CImage::SetDefaults()
{
    ...
    m_pTI = 0;              // pointer to std::type_info class
...
CImage::CImage(CShape s, const std::type_info& ti, int cS)
{
    SetDefaults();
    ReAllocate(s, ti, cS, 0, true, 0);
}

void CImage::ReAllocate(CShape s, const std::type_info& ti, int bandSize,
                        ...
                        void* memory, bool deleteWhenDone, int rowSize)
{
    ...
    m_pTI       = &ti;                      // pointer to std::type_info class
...
void CImage::DeAllocate()
{
    ...
    ReAllocate(CShape(), *(const std::type_info*) 0, 0, 0, false, 0);
    SetDefaults();
}
...
template<>
uchar CImageOf<uchar>::MinVal(void)
{
    return 0;
}

template<>
uchar CImageOf<uchar>::MaxVal(void)
{
    return 255;
}

template<>
int   CImageOf<int  >::MinVal(void)
{
    return -1 ^ (1 << 31);
}

template<>
int   CImageOf<int  >::MaxVal(void)
{
    return  0 ^ (1 << 31);
}

template<>
float CImageOf<float>::MinVal(void)
{
    return FLT_MIN;
}

template<>
float CImageOf<float>::MaxVal(void)
{
    return FLT_MAX;
}
